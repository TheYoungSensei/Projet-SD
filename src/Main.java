...
public class Main {
	...
	private static void plusCourt() {
		...
		String sommetDepart = getStringSommet("Veuillez entrer un sommet de d�part :");
		String sommetArrivee = getStringSommet("Veuilliez entrer le sommet d'arriv�e : ");
		...
		System.out.println("Le chemin est constitu� de " + bfs.getPoidsTotal() + " page(s)");
	}
	...
	private static void moinsCher() {
		String sommetDepart = getStringSommet("Veuillez entrer un sommet de d�part");
		String sommetArrivee = getStringSommet("Veuillez entrer un sommet d'arriv�e");
	...
	private static void test() {
		...
		affichagehttps://status.discordapp.com/Chemin(dijkstra.algorithme(dijkstra.getSommet((int) (Math.random() * (lectureDijkstra.getNombrePages()))),
				dijkstra.getSommet((int) (Math.random() * (lectureDijkstra.getNombrePages())))));
		affichageChemin(dijkstra.algorithme(dijkstra.getSommet((int) (Math.random() * (lectureDijkstra.getNombrePages()))),
				dijkstra.getSommet((int) (Math.random() * (lectureDijkstra.getNombrePages())))));
		affichageChemin(dijkstra.algorithme(dijkstra.getSommet((int) (Math.random() * (lectureDijkstra.getNombrePages()))),
		...
		for(int i = 0; i < 50; i++){
			...
			affichageChemin(dijkstra.algorithme(dijkstra.getSommet((int) (Math.random() * (lectureDijkstra.getNombrePages()))),
					dijkstra.getSommet((int) (Math.random() * (lectureDijkstra.getNombrePages())))));
			System.out.println(
					"Temps en mili : " + (System.currentTimeMillis() - temps) + " Fin de l'it�ration N�" + i);
		}
		for(int i = 0; i < 5; i++){
			long temps = System.currentTimeMillis();
